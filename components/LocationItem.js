...
class LocationItem extends React.Component {
	...
	handleSaveClick = () => {
		...
		if (error.name || error.lat || error.lon)
			return;
		this.props.onSaveClick({ key: this.props.id, name: this.state.name, lat: this.state.lat, lon: this.state.lon, category: this.state.category });
	}
	...
	render() {
		...
		const categoryText = this.state.category === '' ? 'Select category..' : `Category: ${this.state.category}`;
		let categoryTextStyle = this.state.category === '' ? [styles.text, styles.grayedOut] : styles.text;
		let nameInputStyle;
		let latInputStyle;
		let lonInputStyle;
		...
		const saveButton = this.props.inEdit ? <Button title="save" onPress={this.handleSaveClick} /> : null

		return (
			<View style={containerStyle}>
				{nameElement}
				<Collapsible style={styles.collapsible} collapsed={!this.props.selected}>
					<View style={styles.inputRow}>
						<TextInput style={latInputStyle} editable={this.props.inEdit} placeholder="Latitude" keyboardType="numbers-and-punctuation"
							value={this.props.inEdit ? this.state.lat + '' : `Lat: ${this.state.lat}`} onChangeText={this.handleLatChanged}
							onFocus={this.handleLatInputFocus} />
						<View style={styles.spacer} />
						<TextInput style={lonInputStyle} editable={this.props.inEdit} placeholder="Longitude" keyboardType="numbers-and-punctuation"
							value={this.props.inEdit ? this.state.lon + '' : `Lon: ${this.state.lon}`} onChangeText={this.handleLonChanged}
							onFocus={this.handleLonInputFocus} />
					</View>
					<View style={styles.inputRow2}>
						<ModalDropdown disabled={!this.props.inEdit} dropdownTextStyle={styles.dropDownText}
							options={categories} onSelect={(index, value) => this.handleCategoryChanged(index, value)}>
				...
				<Map visible={this.state.mapVisible} lat={this.state.lat} lon={this.state.lon} onCloseAreaClick={this.handleMapCloseAreaClick} inEdit={this.props.inEdit} />
			</View>
		);
	}
}
